...
from schemas import LoadSchema, ProcessSchema, ProcessListSchema
...
def load_datafiles(json_data):
    """Load activity reports for processing."""
    app.logger.info(f"Loading data files...{json_data}")
    payload = json_data

    source = payload.get('source', None)
    fileentry = payload.get('fileentry', None)
    notes = payload.get('notes', None)

    """Load activity reports for processing."""
    print(f"Got source: {source}, file: {fileentry}, notes: {notes}")
    processors = load_sources(source, fileentry)
    if not notes:
        notes = f'{len(processors)} Processors'
...
@app.doc(tags=['Batches'])
@app.input(ProcessListSchema, location='json')
def process_a_transaction_batch_multi(json_data):
    """
    Create a processed batch from the transaction files specified
    """
    app.logger.info(f"Process List payload - 'process_a_transaction_batch' : {json_data}")
    payload = json_data

    file_ids = payload.get('files', None)
    notes = payload.get('notes', None)
    processed_batch_id = db_utils.create_process_batch(notes=notes)

    # --------------------------------------------------------------
    for id in file_ids:
        app.logger.info(
            f"Running transaction batch: {id} into processing batch: {processed_batch_id}"
        )

        # Create a list of processors from the specified batch
        all_processors = select_processors_from_batch(id)
        ...
        for processor_info in all_processors:
            ...
            # app.logger.info(f"cp: {processor_info}")
            cp.match_templates(file_id=id, processed_batch_id=processed_batch_id)
