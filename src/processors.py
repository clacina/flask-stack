...
class SoundChecking(base.ProcessorBase):
    def __init__(self, datafile: str, name: str=None, config=None):
        self.name = name
...
class AmazonReturns(base.ProcessorBase):
    def __init__(self, datafile: str, config=None):
        self.name = "Amazon Returns"
        self.skip_data_rows = 1
        super().__init__(datafile, config, self.name, self.skip_data_rows)

    def parse_datafile(self, datafile):
        """
            "OrderID","ReversalID","RefundCompletionDate","Currency","AmountRefunded","Status","DisbursementType"
        ...
        with open(datafile, "rt", encoding="utf8") as infile:
            ...
            for row in csv_reader:
                print(row)
                data = models.AmazonReturnTransaction()
                data.parse_entry(row)
                self.transactions.append(data)
...
institution_file_mapping = [
    ...
    InstitutionNaming(['home', 'depot'], 'HomeDepot'),
    ...
    InstitutionNaming(['amazon', 'christopher', 'Returns'], 'AmazonReturns'),
    InstitutionNaming(['amazon', 'christopher'], 'AmazonRetail'),
    InstitutionNaming(['amazon', 'christa', 'returns'], 'AmazonReturns'),
