...
def configure_processor(datafile, processor_class, institution_id):
    ...
    """
    current_app.logger.info(f"ConfigProc: {processor_class} for datafile: {datafile}")
    try:
...
def create_batch(processors, notes=None):
    ...
    batch_id = db_utils.create_transaction_batch(notes=notes)
    if not notes:
...
def update_batch(processors, file_id, notes):
    # remove any transactions from the specified batch that are from the processors id
    transactions_processed = 0
    for account in processors:
        db_utils.override_batch_transactions(account.institution_id, account.transactions, file_id)
        account.process_transactions(file_id)
...
def load_sources(source, file):
    ...
    if source:  # load folder
        ...
        new_processor = load_source_file(file)
        if new_processor:
            data.append(new_processor)
    else:
